...
template <> Diligent::float2 Rand<Diligent::float2>() {
    return Diligent::float2(Rand<float>(), Rand<float>());
}

template <> Diligent::float3 Rand<Diligent::float3>() {
    return Diligent::float3(Rand<float>(), Rand<float>(), Rand<float>());
}

template <> Diligent::float4 Rand<Diligent::float4>() {
    return Diligent::float4(Rand<float>(), Rand<float>(), Rand<float>(), Rand<float>());
}

template<typename T> T LinearRand(T Min, T Max) {
    return Rand<T>() * (Max - Min) + Min;
}

template<> uint8_t LinearRand(uint8_t Min, uint8_t Max) {
    return Rand<uint8_t>() * (Max - Min + uint8_t(1)) + Min;
}

template<> uint16_t LinearRand(uint16_t Min, uint16_t Max) {
    return Rand<uint16_t>() * (Max - Min + uint16_t(1)) + Min;
}

template<> uint32_t LinearRand(uint32_t Min, uint32_t Max) {
    return Rand<uint32_t>() * (Max - Min + uint32_t(1)) + Min;
}

template<> uint64_t LinearRand(uint64_t Min, uint64_t Max) {
