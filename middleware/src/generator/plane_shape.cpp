...
PlaneShape::PlaneShape(const UInt2& segments, const dg::Vector2<Axis>& axes)
...
void PlaneShape::FillVertex(VertexBufferRange<VertexPNC>& vb) const {
    ...
    uint32_t axis2 = static_cast<uint32_t>(Axis::X) + static_cast<uint32_t>(Axis::Y) + static_cast<uint32_t>(Axis::Z) - axis0 - axis1;
